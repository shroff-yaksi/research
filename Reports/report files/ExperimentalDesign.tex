...
RE-TabSyn is evaluated on six financial benchmark datasets against four baselines across three evaluation pillars, namely statistical fidelity, machine learning utility, and empirical privacy. All experiments run three random seeds (42, 123, 456); results are reported as mean $\pm$ standard deviation.
...
VAE Encoder is an MLP with hidden dimensions [256, 128] and ReLU activations. Output is the mean vector $\mu \in \mathbb{R}^{64}$ and log-variance vector $\log \sigma^2 \in \mathbb{R}^{64}$.

VAE Decoder uses a mirror architecture [128, 256] with separate output heads, including a linear projection for numerical features and a per-column softmax projection for categorical features.

Diffusion Backbone (DiT) is a Diffusion Transformer with 4 layers, hidden dimension 256, 4 attention heads. The noise schedule uses $T = 1000$ timesteps, linear $\beta$ from $\beta_1 = 10^{-4}$ to $\beta_T = 0.02$.

Guidance uses label dropout $p_{uncond} = 0.10$ during training; guidance scale $w = 2.0$ during inference.

Embeddings use sinusoidal time embeddings projected to 256 dimensions via a two-layer MLP. Learned class embeddings for 3 tokens (class 0, class 1, null $\emptyset$).
...
TVAE uses the same tabular preprocessing as CTGAN, embedding dimension 128, encoder and decoder hidden dimensions [128, 128], 100 epochs. Implemented via SDV.
...
Phase 1, VAE Training (Epochs 1--100)
...
Phase 2, Diffusion Training (Epochs 1--100)
...
\begin{enumerate}
    ...
    \item Each training step involves sampling a timestep $t \sim \mathcal{U}(1, 1000)$ and noise $\boldsymbol{\epsilon} \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$; apply label dropout ($p = 0.10$); compute $\mathbf{z}_t$; predict noise with DiT; backpropagate MSE loss.
...
\]
Values range from 0 (identical distributions) to 1 (completely different). The average KS across all numerical columns is reported. The KS test is non-parametric, as it detects differences anywhere in the distribution, not just in the mean or variance.

Correlation Preservation measures the mean absolute difference between the pairwise Pearson correlation matrices of real and synthetic data.
\[
\text{Corr Error} = \frac{1}{d^2} \sum_{i,j} |r_{ij}^{real} - r_{ij}^{synth}|
\]
This captures joint distribution quality that the KS statistic misses, as two datasets could have identical marginal distributions while having entirely wrong correlation structures.

\subsection{Pillar 2: Machine learning utility}

Low KS does not guarantee a model trained on synthetic data will perform well on real data. The TSTR protocol tests this directly by training on synthetic, test on real, measure the gap.
...
Minority F1-Score is the F1-score for the minority class under the TSTR protocol.
...
Distance to Closest Record (DCR) measures, for each synthetic sample $\tilde{\mathbf{x}}_i$, the Euclidean distance to the nearest real training sample.
\[
