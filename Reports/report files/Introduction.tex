...
\end{itemize}
These regulations simultaneously demand rigorous model validation while restricting the data available for such validation, a tension that synthetic data is well positioned to resolve.
...
We demonstrate that classifiers trained on RE-TabSyn synthetic data achieve a 3.1\% higher minority F1-score (0.472 vs.\ 0.458) than those trained on real imbalanced data, providing the first empirical evidence that guided synthetic data can outperform real data for minority detection tasks.
...
This section delineates the boundaries of the current research and its practical, academic, and societal significance. The scope defines the specific problem domain and data characteristics addressed; the significance describes the broader impact across practice, research, and society.
...
\begin{itemize}
    ...
    \item Moderate dimensionality (tested on datasets with up to 64 features and up to 45,222 samples).
