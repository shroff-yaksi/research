...
CTGAN became the de facto baseline for tabular synthesis, implemented in the widely-used Synthetic Data Vault (SDV) library and reproduced in hundreds of subsequent papers.
...
Ho and Salimans \cite{ho2022cfg} introduced Classifier-Free Guidance (CFG) in 2022, and it quickly became the standard mechanism for controlled generation in image and text domains.
...
Together they triangulate RE-TabSyn's position in the design space: it is better than GANs (CTGAN), better than standalone VAEs (TVAE), better than direct diffusion (TabDDPM), and adds a capability (class control) that the current best-fidelity method (TabSyn) lacks.
