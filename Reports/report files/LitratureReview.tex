...
Ho et al.\ \cite{ho2020ddpm} presented Denoising Diffusion Probabilistic Models, i.e., DDPMs, in 2020. The central idea is both simple and effective: start with real data, gradually add random noise until the data is completely destroyed, and then train a model to reverse that process step by step.
...
TabDDPM \cite{kotelnikov2023tabddpm}, published at ICML 2023 by Kotelnikov et al., was the first systematic application of diffusion models to tabular data. The paper is notable both for its methodology and for documenting the failure modes of applying diffusion naively to heterogeneous data.
...
TabSyn \cite{zhang2024tabsyn}, published at ICLR 2024 by Zhang et al., achieves the highest reported fidelity for tabular data synthesis and directly addresses TabDDPM's categorical encoding problem through a two-stage design:
\begin{enumerate}
...
Ho and Salimans \cite{ho2022cfg} introduced Classifier-Free Guidance, i.e., CFG, in 2022. It became the standard mechanism for controlled generation in image and text domains.
...
Assefa et al.\ \cite{assefa2020synthetic} provided an early survey of synthetic data opportunities and challenges in finance, identifying fraud detection, credit scoring, and stress testing as primary applications. Their survey made clear that the bottleneck was not awareness of the technology but the quality gap: existing synthetic data methods could not reliably preserve the complex statistical properties that financial ML models depend on. Fiore et al.\ \cite{fiore2019hybrid} demonstrated that GAN-augmented training data improved credit card fraud detection accuracy by up to 10\%, providing early empirical support for synthetic data utility in imbalanced financial tasks. The 10\% improvement, while modest, was enough to justify continued investment in the research direction.
