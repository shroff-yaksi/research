...
This literature review surveys peer-reviewed publications spanning synthetic data generation, tabular data modeling, generative adversarial networks, variational autoencoders, diffusion models, and privacy-preserving mechanisms. A systematic analysis of 151 research papers across 10 categories was conducted to identify the state of the art, evaluate existing limitations, and pinpoint the research gap addressed by our proposed approach. Table~\ref{tab:lit_categories} summarizes the scope of this review.
...
\begin{enumerate}
    ...
    \item \textbf{Mode Collapse:} The generator may forget underrepresented modes of the data distribution, a direct failure for rare event generation. A GAN on a dataset with 5\% fraud can collapse to generating only the majority pattern, ignoring fraud entirely.
...
\begin{enumerate}
    ...
    \item \textbf{Flexible Conditioning:} Conditioning information can be incorporated at each denoising step, enabling control over generated outputs -- the property RE-TabSyn exploits through CFG.
\end{enumerate}
...
Any comparative study requires a deliberate choice of baselines. Including too few makes the comparison insufficient; including too many obscures the narrative. RE-TabSyn is compared against four baselines: CTGAN, TVAE, TabDDPM, and TabSyn. Each was chosen for a specific reason, and together they span the space of relevant prior work.

\subsection{CTGAN as GAN representative}

CTGAN \cite{xu2019ctgan} is the standard for tabular data synthesis. It has been cited over 2,000 times, is the default model in the Synthetic Data Vault, i.e., SDV, library that financial institutions use in practice, and has been reproduced independently across hundreds of publications. Any claimed improvement in tabular synthesis must beat CTGAN, or the comparison is incomplete. Including it anchors the fidelity axis of the comparison: CTGAN achieves KS $\approx$ 0.153, which is a known reference point.
...
TabDDPM \cite{kotelnikov2023tabddpm} is included as a negative baseline. Its inclusion is not to show a close competitor but to demonstrate a failure mode that RE-TabSyn's architecture specifically addresses.
...
Without TabDDPM, RE-TabSyn's architectural choice of latent-space diffusion would appear to be one option among many. With TabDDPM included, the comparison shows it to be a necessary design decision.

\subsection{TabSyn as state-of-the-art reference}

TabSyn \cite{zhang2024tabsyn} is the highest-fidelity baseline, published at ICLR 2024. It is the method that RE-TabSyn builds directly upon. Including it is both an obligation and a deliberate positioning choice.
...
\begin{enumerate}
    \item \textbf{Gap 1: Absence of Controllable Minority Class Generation.} No existing method enables practitioners to specify desired minority ratios during generation. All methods replicate (or degrade) the training class distribution without any mechanism for adjustment. This means synthetic data cannot address class imbalance, which is the primary reason financial institutions would want to use it. A fraud detection team that generates synthetic data with TabSyn still ends up with the same 0.5\% fraud prevalence as their real data. They would need to generate 200 times as much data to get the same number of fraud cases as legitimate ones, which creates its own statistical problems.

    \item \textbf{Gap 2: CFG Application to Tabular Domain.} Classifier-Free Guidance has proven effective in image synthesis and became the default conditioning mechanism for leading text-to-image models. Its ability to control class distribution at inference time, without requiring retraining or auxiliary classifiers, makes it well-suited to the minority ratio control problem. Its potential for structured tabular data class control remained entirely unexplored. Our systematic search across all 151 papers found zero instances of CFG applied to tabular synthesis. The adaptation is non-trivial: image CFG operates on continuous pixel values, while tabular data has mixed types, different scales, and complex inter-column dependencies that require careful architectural design.
...
\begin{enumerate}
    \item Adopting latent space diffusion for mixed-type handling, borrowing TabSyn's core architectural insight.
