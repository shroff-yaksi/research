...
Denoising Diffusion Probabilistic Models, i.e., DDPMs, were presented in 2020 \cite{ho2020ddpm}. The central idea is both simple and effective, starting with real data, gradually add random noise until the data is completely destroyed, and then train a model to reverse that process step by step.
...
\begin{enumerate}
    \item A VAE encodes the mixed-type tabular data (numerical and categorical columns together) into a smooth, continuous latent representation where all features are represented as real-valued vectors. The discreteness problem disappears because instead of working with raw category labels, the diffusion model works with the VAE's learned continuous encodings.
...
The parameter $w$ is the guidance scale. At $w = 1$, you get standard conditional generation. At $w = 2$ (the default in RE-TabSyn), the model is pushed twice as far in the direction of the desired class, producing a strong class signal while still generating realistic-looking samples. This guidance scale provides an intuitive trade-off, where higher $w$ produces outputs more aligned with the condition but potentially less diverse.

CFG became the standard conditioning mechanism for text-to-image models (Stable Diffusion, DALL-E 2, Imagen). It requires no separate classifier training, no modifications to the base architecture, and can be applied at inference time without retraining. This last property matters for RE-TabSyn because a practitioner can train the model once on the real imbalanced data and then generate datasets with any desired minority ratio at runtime.
...
\begin{enumerate}
    ...
    \item Gap 3. No unified fidelity-privacy-control framework. Methods optimize for one objective at a time, with TabSyn for fidelity, DP-CTGAN for formal privacy, SMOTE-based methods for class balance. No single prior method addresses all three simultaneously.
\end{enumerate}

RE-TabSyn addresses all three gaps, as it adopts latent space diffusion for mixed-type handling (borrowing TabSyn's core architectural insight), introduces Classifier-Free Guidance to tabular synthesis for the first time, and maintains competitive fidelity alongside controllability. The cost is a modest reduction in peak fidelity (KS = 0.171 versus TabSyn's 0.109), representing the trade-off between tight distributional matching and class-conditional control. Chapter~5 argues that this trade-off is acceptable, and in imbalanced settings favorable, as the utility gain from balanced training data outweighs the small fidelity loss.
