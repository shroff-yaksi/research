...
The core learning objective is to learn a generative model $G_\theta$ that synthesizes samples $\tilde{\mathbf{x}} \sim p_\theta(\mathbf{x})$ indistinguishable from $\mathbf{x} \sim p_{data}(\mathbf{x})$, with the additional capability to control the class distribution $p_\theta(y)$ at generation time. This is a stricter objective than standard generative modeling, as the target is samples whose class composition can be specified by the user after training, not merely realistic samples. Four criteria guide the framework design.
\begin{enumerate}
...
Training dynamics are notoriously unstable, as a discriminator that is too strong causes vanishing gradients for $G$, while one that is too weak provides no learning signal. Techniques such as Wasserstein distance \cite{arjovsky2017wgan} and spectral normalization help, but do not fully resolve the problem.
...
\begin{enumerate}
    \item Variational Autoencoder (VAE), for mixed-type encoding into continuous latent space
    \item Latent Diffusion Model (LDM), for generation in the smooth continuous latent space
    \item Classifier-Free Guidance (CFG), for controllable class ratios at inference time
\end{enumerate}
...
The DiT architecture consists of the following.
\begin{enumerate}
...
When $w > 1$, probability mass concentrates in regions where $p(\mathbf{z}|y=1)$ is high relative to $p(\mathbf{z})$, which are the regions corresponding to minority class features. The guidance scale $w$ provides continuous, inference-time control over class bias without retraining.


\subsection{The fidelity-control trade-off}

Increasing $w$ boosts minority representation but introduces a distributional shift, where the guided distribution $\tilde{p}$ deviates further from the true data distribution as $w$ increases.
...
\hline
MLP encoder [256, 128] & Compact feature compression with stable gradients & Transformer encoder offers better quality but considerably higher training cost; deferred to future work \\
\hline
$\beta = 0.1$ KL weight & Prevents posterior collapse without over-regularizing & $\beta = 1.0$ (standard VAE) causes posterior collapse on small datasets; $\beta = 0.01$ leads to insufficient regularization \\
\hline
...
\hline
4 DiT layers, dim 256 & Balance capacity and overfitting risk for datasets up to 45K rows & More layers lead to overfitting on small datasets; fewer layers provide insufficient capacity \\
\hline
$T = 1000$ timesteps & Smooth noise trajectory for stable training and generation & $T = 100$ is too coarse with worse quality; $T = 2000$ yields no quality gain, noticeably slower generation \\
\hline
Linear $\beta$ schedule & Proven stable across diverse datasets & Cosine schedule is marginally better for images, no benefit observed for tabular data \\
\hline
...
\hline
Guidance scale $w = 2.0$ & Near-50\% minority ratio with acceptable fidelity cost & Higher $w$ ($>$3) overshoots 50\% and degrades fidelity; lower $w$ ($<$1) provides insufficient minority boost \\
\hline
Two-phase training & Decouples representation learning from generative modeling & Joint training causes latent space instability degrades both objectives simultaneously \\
\hline
Frozen VAE in Phase 2 & Prevents latent space drift during diffusion training & Unfrozen VAE allows latent space changes invalidate already-trained diffusion parameters \\
\hline
