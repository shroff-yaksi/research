...
\begin{itemize}
    ...
    \item \textbf{High-DCR datasets} (Lending Club: 4,986; Polish Bankruptcy: 1,250) indicate excellent privacy: synthetic samples are far from any real record.
...
\begin{itemize}
    ...
    \item \textbf{Small and extreme datasets show wider variance.} German Credit (44.8\% $\pm$ 2.1\%) and Polish Bankruptcy (47.8\% $\pm$ 3.2\%) exhibit more deviation, attributed to smaller training sets (1,000 and 5,000 samples respectively) and extreme original imbalance (30\% and 4.8\%).
    \item \textbf{The boost magnitude is substantial.} Polish Bankruptcy is boosted from 4.8\% to 47.8\%, a 10$\times$ increase in minority representation. Bank Marketing is boosted from 11.3\% to 50.2\%, a 4.4$\times$ increase. This demonstrates RE-TabSyn's ability to handle even extreme imbalance.
...
RE-TabSyn achieves the \textbf{best minority F1-score} (0.472, surpassing even real data), \textbf{precise minority control} (within 1.7\% of target), and \textbf{strong privacy} (DCR $>$ 1.0), while maintaining \textbf{competitive fidelity} (KS = 0.171). No other method achieves controllable minority generation; RE-TabSyn fills a critical gap in the synthetic tabular data field.
