...
\author{\IEEEauthorblockN{1\textsuperscript{st} Yaksi Ketan Shroff}
...
\and
\IEEEauthorblockN{2\textsuperscript{nd} Dr. Vishvajit Bakrola}
\IEEEauthorblockA{\textit{Associate Professor \& Director} \\
\textit{Asha M. Tarsadia Institute of Computer Science and Technology}\\
Uka Tarsadia University, Bardoli, India \\
...
Generating synthetic records seemed, at first glance, like a natural antidote. The approach appears straightforward: if real data is scarce or imbalanced, synthetic data could provide realistic substitutes. Ideally, synthetic data could serve dual purposes: bolstering scarce minority classes to improve model boundaries, and sidestepping rigorous privacy constraints (like GDPR~\cite{gdpr2016regulation} or CCPA) when sharing datasets across organizations~\cite{figueira2022survey}.

However, reality has proven far messier. The current generation of state-of-the-art generative models—ranging from GANs to Diffusion Models—are designed with a single objective: to faithfully absorb and reproduce the underlying probability distribution of the training data~\cite{jordon2022fake}. They function as faithful replicators of training distributions. If the training data contains 1\% fraud, a well-trained generator will produce synthetic data with exactly 1\% fraud. Requesting balanced output from these models is fundamentally incompatible with their design objective of faithful distribution replication.
...
Simultaneously, the high-impact phenomena banks and insurers care most about—chargebacks, policy lapses, insolvency events—remain persistently rare~\cite{fiore2019hybrid, nidhi2024rare}. Traditional resampling heuristics like SMOTE~\cite{chawla2002smote} offer partial relief by interpolating between existing minority points, but they do not conjure authentic-looking new minority instances from the underlying data manifold, often creating noisy or unrealistic samples that confuse decision boundaries.

\subsection{Core Idea: Classifier-Free Guidance}
The framework introduced here, dubbed \retabsyn{} (Rare-Event Enhanced Tabular Synthesis), tackles this shortcoming by borrowing a powerful idea from the domain of conditional image generation: Classifier-Free Guidance (CFG)~\cite{ho2022cfg}. CFG has proved to be a key enabler of recent advances in text-to-image systems like DALL-E 2 and Stable Diffusion, where it allows the model to steer diffusion sampling toward specific text prompts without requiring massive auxiliary classifier networks.
...
Variational Autoencoders~\cite{kingma2014vae} model data through latent variables. As formulated by Kingma and Welling~\cite{kingma2014vae}, the training objective maximizes the Evidence Lower Bound (ELBO):
\begin{equation}
...
Tabular adaptations like TVAE~\cite{xu2019ctgan} offer smoother optimization landscapes compared to GANs. However, the Gaussian decoder assumption produces samples with reduced fidelity---in the tabular domain, this manifests as noisy categorical features and poor discrete distribution capture.

\subsection{Diffusion-Based Generation}
The diffusion paradigm, pioneered by Ho et al.~\cite{ho2020ddpm}, learns to reverse a gradual noise-injection process. Following Ho et al.~\cite{ho2020ddpm}, the forward process adds Gaussian noise over $T$ timesteps:
\begin{equation}
...
Our mean KS statistic across datasets (0.171) is somewhat higher than the unguided \tabsyn{} (0.109). This discrepancy is logical: \tabsyn{} is optimized solely to copy the distribution perfectly. We are intentionally biasing it. However, our fidelity remains competitive with popular baselines like CTGAN (0.153) and TVAE (0.165), suggesting the synthesized data is still structurally sound and realistic enough for use.
...
\subsection{Usage of RE-TabSyn}
This tool is not a universal silver bullet. If your goal is to publish a dataset that statistically mirrors the original in every way, for example, for a public transparency report, \tabsyn{} remains the gold standard for fidelity. However, if your goal is to \textit{train a model to find needles in a haystack}, \retabsyn{} appears to be the superior choice. It acts as a sophisticated oversampling technique that does not just copy old points (like SMOTE), but instead generates new minority-class variations that are consistent with the learned latent structure of the data.
...
\begin{table}[htbp]
\caption{Computational cost comparison on the Adult dataset.}
\label{tab:compute}
\centering
\small
\begin{tabular}{lccc}
...
\begin{table}[htbp]
\caption{Ablation study results on the Adult dataset.}
\label{tab:ablation}
\centering
\small
\begin{tabular}{lccc}
...
We have introduced \retabsyn{}, a latent-diffusion framework that gives data scientists a practical ``volume knob'' for class ratios in tabular data. By adapting Classifier-Free Guidance to the tabular domain, practitioners can specify target minority ratios rather than accept the scarcity dictated by the training distribution. Across six financial datasets spanning substantial imbalance, \retabsyn{} reliably generated near-balanced synthetic data and improved downstream detection of rare events such as fraud, default, and bankruptcy. While guidance introduces an expected fidelity--control trade-off, the samples remain structurally consistent enough to yield gains under TSTR evaluation, indicating that controllability can be achieved without abandoning realistic latent structure. Overall, \retabsyn{} serves as a principled augmentation alternative to heuristic oversampling for minority-critical financial modeling, and it helps bridge the gap between high-fidelity generators and task-driven data augmentation. Promising extensions include privacy-preserving training and richer conditioning (multi-class, temporal, and constraint-aware generation).
